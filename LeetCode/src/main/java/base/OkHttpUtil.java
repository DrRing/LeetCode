...
public class OkHttpUtil {
	...
	public static String postJson(String path, String json, Integer connectionTime, Integer readTime) {
		...
		client.newBuilder().connectTimeout(connectionTime, TimeUnit.SECONDS).readTimeout(readTime, TimeUnit.SECONDS)
				.build();
	 ...
	 * @param readTime       设置读取超时时间
	 */
	public static String postJsonWithConnectionTime(String path, String json, Long connectionTime, Long readTime,
			Long writeTime) {
		OkHttpClient client = new OkHttpClient.Builder()
//		        .connectTimeout(60 * 1000, TimeUnit.MILLISECONDS)
//		        .readTimeout(5 * 60 * 1000, TimeUnit.MILLISECONDS)
//		        .writeTimeout(5 * 60 * 1000, TimeUnit.MILLISECONDS)
				.connectTimeout(connectionTime, TimeUnit.MILLISECONDS).readTimeout(readTime, TimeUnit.MILLISECONDS)
				.writeTimeout(writeTime, TimeUnit.MILLISECONDS).build();
		RequestBody body = RequestBody.create(jsonMediaType, json);
		// 3 创建请求方式
		Request request = new Request.Builder().url(path).post(body).build();
		try {
